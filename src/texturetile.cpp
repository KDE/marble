...
//	slotLoadTile( "test/2/0003/0003_0004.jpg" );
}

void TextureTile::loadTile( int x, int y, int level, const QString& theme, bool requestRepaint ){
	m_used=true;
	QString absfilename;
//	qDebug() << "Requested tile level" << level;

//	If the tile level offers the requested tile then load it. Otherwise cycle from 
//	the requested tilelevel down to one where the requested area is covered. Then 
//	scale the area to create a replacement for the tile that has been requested.
