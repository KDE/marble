...
int EclSolar::getEclYearInfo(int k, int &yr, int &month, int &day, int &hour, int &min, double &sec, double &tzone, double &magn)
{
...
double EclSolar::phmjd(double yearf, double phase, double tdut, int &eph, double &ejd, double &emag)
/*
...
*/
{
    ...
    jd = 15020.25933 + 29.53058868 * k + (0.0001178 - 0.000000155 * tt) * tt * tt + 0.00033 * sin(degrad * jd);

    eph = 0; // assume no eclipse

    // Sun's mean anomaly
    m = degrad * (359.2242 + 29.10535608 * k - (0.0000333 - 0.00000347 * tt) * tt * tt);
    // Moon's mean anomaly
    p = degrad * (306.0253 + 385.81691808 * k + (0.0107306 + 0.00001236 * tt) * tt * tt);
    // 2* Moon's argument of latitude
    f = 2.0 * degrad * (21.2964 + 390.67050646 * k - (0.0016528 - 0.00000239 * tt) * tt * tt);

    if ((phase == 0) || (phase == 0.5)) // for Full and New Moon
    {
        k = (0.1734 - 0.000393 * tt) * sin(m) + 0.0021 * sin(2.0 * m) - 0.4068 * sin(p) + 0.0161 * sin(2.0 * p) - 0.0004 * sin(3.0 * p) + 0.0104 * sin(f)
            - 0.0051 * sin(m + p) - 0.0074 * sin(m - p) + 0.0004 * sin(f + m) - 0.0004 * sin(f - m) - 0.0006 * sin(f + p) + 0.0010 * sin(f - p)
            + 0.0005 * sin(m + 2.0 * p);
        ...
        {
            ejd = (0.1734 - 0.000393 * tt) * sin(m) + 0.0021 * sin(2.0 * m) - 0.4068 * sin(p) + 0.0161 * sin(2.0 * p) - 0.0051 * sin(m + p)
                - 0.0074 * sin(m - p) - 0.0104 * sin(2.0 * f);
            ejd += jd;

            s = 5.19595 - 0.0048 * cos(m) + 0.0020 * cos(2.0 * m) - 0.3283 * cos(p) - 0.0060 * cos(m + p) + 0.0041 * cos(m - p);

            c = 0.2070 * sin(m) + 0.0024 * sin(2.0 * m) - 0.0390 * sin(p) + 0.0115 * sin(2.0 * p) - 0.0073 * sin(m + p) - 0.0067 * sin(m - p)
                + 0.0117 * sin(2.0 * f);

            gam = s * sin(f) + c * cos(f);
            u = 0.0059 + 0.0046 * cos(m) - 0.0182 * cos(p) + 0.0004 * cos(2.0 * p) - 0.0005 * cos(m + p);
    ...
    {
        k = (0.1721 - 0.0004 * tt) * sin(m) + 0.0021 * sin(2.0 * m) - 0.6280 * sin(p) + 0.0089 * sin(2.0 * p) - 0.0004 * sin(3.0 * p) + 0.0079 * sin(f)
            - 0.0119 * sin(m + p) - 0.0047 * sin(m - p) + 0.0003 * sin(f + m) - 0.0004 * sin(f - m) - 0.0006 * sin(f + p) + 0.0021 * sin(f - p)
            + 0.0003 * sin(m + 2.0 * p) + 0.0004 * sin(m - 2.0 * p) - 0.0003 * sin(2.0 * m + p);
        if (phase == 0.25) {
...
void EclSolar::ckphase(double minmjd, double maxmjd, double yr, double deltdut, int &mp, PMJD p, double phase)
    ...
    if (eph != 0) {
        ...
        if ((td >= minmjd) && (td <= maxmjd) && (eb_numecl < GBL_ECLBUF)) {
            if (eb_numecl == 0) {
                ...
                for (j = 0; j < eb_numecl; j++) { // check whether the date is already stored
...
void EclSolar::moonph()
{
    ...
    while (yr < yx) { // calculate and check the phases
...
double EclSolar::getlocmag(double jd, double ep2, double phi, double lamda, double height, const Vec3 &rs, const Vec3 &rm, int &totflg)
{
...
int EclSolar::iscrs(double vrc0, double vrc1, double dpn, double &vrx0, double &vrx1, double &vrx20, double &vrx21)
{
...
int EclSolar::localStart(int j, double *spt, double *ept, int *spp, int p, char *otxt)
{
    /* get start and end times of the various phases of the eclipse
       j = index of the eclipse
    ...
    do {
        ...
        if (pcur > 0) {
            AppPos(jd, eclp.GetEp2(), phi, lamda, eb_geoheight, 1, gx, azim, elev, dist);
            if ((p > 0) && (elev >= elrise)) {
                magecl = getlocmag(jd, eclp.GetEp2(), phi, lamda, eb_geoheight, eclp.GetRSun(), eclp.GetRMoon(), i);
                if (magecl > eb_maxps) {
                    eb_maxps = magecl;
                    eb_maxelv = elev;
                    eb_jdmaxps = jd;
...
void EclSolar::getLocalDetails(char *otxt)
{
    ...
    */
    ...
    if ((p > 0) && (eb_lccnt > 0)) {
        ...
        strcat(otxt, outb);
        if (eb_ltotb <= eb_ltote) { // local central eclipse
            if ((p % 2) == 0)
                strcpy(outb, "\nannularity from");
            else
                strcpy(outb, "\ntotality from");
