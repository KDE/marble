...
Mat3 pmatecl(double t1, double t2) //  ecl. precession
{
    ...
    dt = t2 - t1;
    ppi = 174.876383889 + (((3289.4789 + 0.60622 * t1) * t1) + ((-869.8089 - 0.50491 * t1) + 0.03536 * dt) * dt) / 3600.0;
    ppi = ppi * 1.74532925199e-2;
    pii = ((47.0029 - (0.06603 - 0.000598 * t1) * t1) + ((-0.03302 + 0.000598 * t1) + 0.00006 * dt) * dt) * dt * secrad;
    pa = ((5029.0966 + (2.22226 - 0.000042 * t1) * t1) + ((1.11113 - 0.000042 * t1) - 0.000006 * dt) * dt) * dt * secrad;
...
Mat3 pmatequ(double t1, double t2) //  equ. precession
{
    ...
    dt = t2 - t1;
    zeta = ((2306.2181 + (1.39656 - 0.000139 * t1) * t1) + ((0.30188 - 0.000345 * t1) + 0.017998 * dt) * dt) * dt * secrad;
    z = zeta + ((0.7928 + 0.000411 * t1) + 0.000205 * dt) * dt * dt * secrad;
    theta = ((2004.3109 - (0.8533 + 0.000217 * t1) * t1) - ((0.42665 + 0.000217 * t1) + 0.041833 * dt) * dt) * dt * secrad;
...
Mat3 nutmat(double t, double &ep2, bool hpr)
{
    ...
    if (hpr) {
        lm = 2.355548393544 + (8328.691422883903 + (0.000151795164 + 0.000000310281 * t) * t) * t;
        ls = 6.240035939326 + (628.301956024185 + (-0.000002797375 - 0.000000058178 * t) * t) * t;
        f = 1.627901933972 + (8433.466158318464 + (-0.000064271750 + 0.000000053330 * t) * t) * t;
        d = 5.198469513580 + (7771.377146170650 + (-0.000033408511 + 0.000000092115 * t) * t) * t;
        n = 2.182438624361 + (-33.757045933754 + (0.000036142860 + 0.000000038785 * t) * t) * t;
    ...
    else // low precision
    {
        ...
        dpsi = (-17.2 * sin(n) - 1.319 * sin(2 * (f - d + n)) - 0.227 * sin(2 * (f + n)) + 0.206 * sin(2 * n) + 0.143 * sin(ls)) * secrad;
        deps = (+9.203 * cos(n) + 0.574 * cos(2 * (f - d + n)) + 0.098 * cos(2 * (f + n)) - 0.09 * cos(2 * n)) * secrad;
    };
...
void AppPos(double jd, double ep2, double lat, double lng, double ht, int solsys, const Vec3 &r, double &azim, double &elev, double &dist)
{
...
void Sun200::state(double t, Vec3 &rs, Vec3 &vs)
{
    ...
    dl = dl + 6.4 * sin(p2 * (0.6983 + 0.0561 * t)) + 1.87 * sin(p2 * (0.5764 + 0.4174 * t)) + 0.27 * sin(p2 * (0.4189 + 0.3306 * t))
...
void Sun200::addthe(double c1, double s1, double c2, double s2, double &cc, double &ss)
{
    cc = c1 * c2 - s1 * s2;
    ss = s1 * c2 + c1 * s2;
}

void Sun200::term(int i1, int i, int it, double dlc, double dls, double drc, double drs, double dbc, double dbs)
{
    if (it == 0)
...
void Moon200::addthe(double c1, double s1, double c2, double s2, double &c, double &s)
{
    c = c1 * c2 - s1 * s2;
    s = s1 * c2 + c1 * s2;
}
...
void Moon200::long_periodic(double t)
{
    ...
    dd = dl0 - dls;
    dgam = -3332E-9 * sinus(0.59734 - 5.37261 * t) - 539E-9 * sinus(0.35498 - 5.37899 * t) - 64E-9 * sinus(0.39943 - 5.37511 * t);
}
...
void Moon200::addsol(double coeffl, double coeffs, double coeffg, double coeffp, int p, int q, int r, int s)
{
    double x, y;
    term(p, q, r, s, x, y);
    dlam = dlam + coeffl * y;
...
void Moon200::addn(double coeffn, int p, int q, int r, int s, double &n, double &x, double &y)
{
    term(p, q, r, s, x, y);
    n = n + coeffn * y;
}
...
void Moon200::planetary(double t)
{
    ...
    dlam = dlam + 0.82 * sinus(0.7736 - 62.5512 * t) + 0.31 * sinus(0.0466 - 125.1025 * t) + 0.35 * sinus(0.5785 - 25.1042 * t)
        + 0.66 * sinus(0.4591 + 1335.8075 * t) + 0.64 * sinus(0.313 - 91.568 * t) + 1.14 * sinus(0.148 + 1331.2898 * t) + 0.21 * sinus(0.5918 + 1056.5859 * t)
        + 0.44 * sinus(0.5784 + 1322.8595 * t) + 0.24 * sinus(0.2275 - 5.7374 * t) + 0.28 * sinus(0.2965 + 2.6929 * t) + 0.33 * sinus(0.3132 + 6.3368 * t);
}
...
void Eclipse::penumd(double jd, double tdut, Vec3 &vrm, Vec3 &ves, double &dpn, double &pang)
{
...
void Eclipse::umbra(double jd, double tdut, Vec3 &vrm, Vec3 &ves, double &dpn, double &pang)
{
    /* Calculates various items needed for finding out the northern and
