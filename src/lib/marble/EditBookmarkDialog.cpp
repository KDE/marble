...
void EditBookmarkDialogPrivate::initialize()
{
    ...
    m_ui.m_header->setIconLink(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/bookmark.png")));
...
EditBookmarkDialog::EditBookmarkDialog(BookmarkManager *bookmarkManager, QWidget *parent)
    : QDialog(parent)
    , d(new EditBookmarkDialogPrivate(this, bookmarkManager))
{
    d->initialize();
}
...
void EditBookmarkDialog::setMarbleWidget(MarbleWidget *widget)
{
    ...
    QObject::connect(d->m_manager,
                     SIGNAL(reverseGeocodingFinished(GeoDataCoordinates, GeoDataPlacemark)),
                     this,
                     SLOT(retrieveGeocodeResult(GeoDataCoordinates, GeoDataPlacemark)));
}
...
void EditBookmarkDialogPrivate::retrieveGeocodeResult(const GeoDataCoordinates &coordinates, const GeoDataPlacemark &placemark)
{
    ...
    if (distance >= 3500) {
        ...
        bookmarkName = append(data.value(QStringLiteral("city")).value().toString(), data.value(QStringLiteral("state")).value().toString());
        bookmarkName = append(bookmarkName, data.value(QStringLiteral("country")).value().toString());
    } else {
        bookmarkName = append(data.value(QStringLiteral("road")).value().toString(), data.value(QStringLiteral("city")).value().toString());
        bookmarkName = append(bookmarkName, data.value(QStringLiteral("country")).value().toString());
    }
