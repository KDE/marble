...
namespace dgml
{
DGML_DEFINE_TAG_HANDLER(Lakes)

GeoNode* DgmlLakesTagHandler::parse(GeoParser& parser) const
{
    // Check whether the tag is valid
    Q_ASSERT(parser.isStartElement() && parser.isValidElement(dgmlTag_Lakes));
    ...
    if ( parentItem.represents( dgmlTag_Filter ) ) {
        ...
        filter->setLakes( parser.readElementText().trimmed() );
