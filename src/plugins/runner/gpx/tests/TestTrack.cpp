...
void TestTrack::extendedDataHeartRateTest()
{
    ...
    QString content(
        QStringLiteral("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>"
    ...
    {
        GeoDataSimpleArrayData *hr = track->extendedData().simpleArrayData(QStringLiteral("heartrate"));
        QCOMPARE(hr->size(), 3);
        QCOMPARE(hr->valueAt(0), QVariant(QStringLiteral("108")));
        QCOMPARE(hr->valueAt(2), QVariant(QStringLiteral("110")));
    }
