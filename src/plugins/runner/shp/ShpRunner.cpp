    ...
    for (int i = 0; i < entities; ++i) {
        ...
        case SHPT_MULTIPOINT: {
            ...
            for (int j = 0; j < shape->nVertices; ++j) {
                geom->append(new GeoDataPoint(GeoDataCoordinates(shape->padfX[j], shape->padfY[j], 0, GeoDataCoordinates::Degree)));
            }
            ...
            if (shape->nParts != 1) {
                ...
                for (int j = 0; j < shape->nParts; ++j) {
                    ...
                    for (int k = shape->panPartStart[j]; k < itEnd; ++k) {
                        line->append(GeoDataCoordinates(shape->padfX[k], shape->padfY[k], 0, GeoDataCoordinates::Degree));
                    }
                    geom->append(line);
                }
                placemark->setGeometry(geom);
                ...
                GeoDataLineString *line = new GeoDataLineString;
                for (int j = 0; j < shape->nVertices; ++j) {
                    line->append(GeoDataCoordinates(shape->padfX[j], shape->padfY[j], 0, GeoDataCoordinates::Degree));
                }
            ...
            if (shape->nParts != 1) {
                ...
                for (int j = 0; j < shape->nParts; ++j) {
                    ...
                    for (int k = itStart; k < itEnd; ++k) {
                        ring.append(GeoDataCoordinates(shape->padfX[k], shape->padfY[k], 0, GeoDataCoordinates::Degree));
                    }
                ...
                GeoDataLinearRing ring;
                for (int j = 0; j < shape->nVertices; ++j) {
                    ring.append(GeoDataCoordinates(shape->padfX[j], shape->padfY[j], 0, GeoDataCoordinates::Degree));
                }
                poly->setOuterBoundary(ring);
                placemark->setGeometry(poly);
