...
void MapScaleFloatItem::setProjection(const ViewportParams *viewport)
{
    ...
    if (!(m_radius == viewport->radius() && viewportWidth == m_viewportWidth && m_target == target && m_scaleInitDone)) {
        int fontHeight = QFontMetrics(font()).ascent();
        if (m_showRatioScale) {
            setContentSize(QSizeF(viewport->width() / m_widthScaleFactor, fontHeight + 3 + m_scaleBarHeight + fontHeight + 7));
        } else {
...
void MapScaleFloatItem::paintContent(QPainter *painter)
{
    ...
    painter->drawRect(m_leftBarMargin, fontHeight + 3, m_bestDivisor * m_pixelInterval, m_scaleBarHeight);

    painter->setPen(QColor(Oxygen::aluminumGray4));
    painter->drawLine(m_leftBarMargin + 1,
                      fontHeight + 2 + m_scaleBarHeight,
                      m_leftBarMargin + m_bestDivisor * m_pixelInterval - 1,
                      fontHeight + 2 + m_scaleBarHeight);
    painter->setPen(QColor(Qt::black));
    ...
    for (int j = 0; j <= m_bestDivisor; j += 2) {
        if (j < m_bestDivisor) {
            painter->drawRect(m_leftBarMargin + j * m_pixelInterval, fontHeight + 3, m_pixelInterval - 1, m_scaleBarHeight);

            painter->setPen(QColor(Oxygen::aluminumGray5));
            painter->drawLine(m_leftBarMargin + j * m_pixelInterval + 1, fontHeight + 4, m_leftBarMargin + (j + 1) * m_pixelInterval - 1, fontHeight + 4);
            painter->setPen(QColor(Qt::black));
        }
        ...
        if (j == m_bestDivisor) {
            currentStringBegin = (j * m_pixelInterval - QFontMetrics(font()).boundingRect(intervalStr).width());
        } else {
            currentStringBegin = (j * m_pixelInterval - QFontMetrics(font()).horizontalAdvance(intervalStr) / 2);
        }
...
void MapScaleFloatItem::calcScaleBar()
{
    ...
    m_pixelInterval = (int)(m_scaleBarWidth * (qreal)(bestMagValue) / (qreal)(magValue) / m_bestDivisor);
...
void MapScaleFloatItem::contextMenuEvent(QWidget *w, QContextMenuEvent *e)
{
    if (!m_contextMenu) {
        ...
        QAction *toggleAction = m_contextMenu->addAction(tr("&Ratio Scale"), this, SLOT(toggleRatioScaleVisibility()));
