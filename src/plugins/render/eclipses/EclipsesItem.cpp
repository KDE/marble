...
{
    return ((m_startDatePartial <= dateTime) && (m_endDatePartial >= dateTime));
}
...
QString EclipsesItem::phaseText() const
{
    switch (m_phase) {
    case TotalMoon:
        return tr("Moon, Total");
    case PartialMoon:
        return tr("Moon, Partial");
    case PenumbralMoon:
        return tr("Moon, Penumbral");
    case PartialSun:
        return tr("Sun, Partial");
    case NonCentralAnnularSun:
        return tr("Sun, non-central, Annular");
    case NonCentralTotalSun:
        return tr("Sun, non-central, Total");
    case AnnularSun:
        return tr("Sun, Annular");
    case TotalSun:
        return tr("Sun, Total");
    case AnnularTotalSun:
...
{
    return (m_endDatePartial.toSecsSinceEpoch() - m_startDatePartial.toSecsSinceEpoch()) / 3600;
}
...
void EclipsesItem::initialize()
{
    ...
    phase = m_ecl->getEclYearInfo(m_index, year, month, day, hour, min, secs, tz, m_magnitude);

    switch (phase) {
    case -4:
        m_phase = EclipsesItem::TotalMoon;
        break;
    case -3:
        m_phase = EclipsesItem::PartialMoon;
        break;
    case -2:
    case -1:
        m_phase = EclipsesItem::PenumbralMoon;
        break;
    case 1:
        m_phase = EclipsesItem::PartialSun;
        break;
    case 2:
        m_phase = EclipsesItem::NonCentralAnnularSun;
        break;
    case 3:
        m_phase = EclipsesItem::NonCentralTotalSun;
        break;
    case 4:
        m_phase = EclipsesItem::AnnularSun;
        break;
    case 5:
        m_phase = EclipsesItem::TotalSun;
        break;
    case 6:
        m_phase = EclipsesItem::AnnularTotalSun;
        break;
    default:
        mDebug() << "Invalid phase for eclipse at" << year << "/" << day << "/" << month << "!";
    }

    m_dateMaximum = QDateTime(QDate(year, month, day), QTime(hour, min, secs), Qt::LocalTime);
    ...
    if (m_ecl->getPartial(mjd_start, mjd_end) != 0) {
        ...
        m_startDatePartial = QDateTime(QDate(year, month, day), QTime(hour, min, secs), Qt::LocalTime);
        m_ecl->getDatefromMJD(mjd_end, year, month, day, hour, min, secs);
        m_endDatePartial = QDateTime(QDate(year, month, day), QTime(hour, min, secs), Qt::LocalTime);
    } else {
    ...
    if (m_isTotal) {
        m_ecl->getDatefromMJD(mjd_start, year, month, day, hour, min, secs);
        m_startDateTotal = QDateTime(QDate(year, month, day), QTime(hour, min, secs), Qt::LocalTime);
        m_ecl->getDatefromMJD(mjd_end, year, month, day, hour, min, secs);
        m_endDateTotal = QDateTime(QDate(year, month, day), QTime(hour, min, secs), Qt::LocalTime);
    }
