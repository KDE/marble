...
{
    QFile names(MarbleDirs::dataFilePath(QStringLiteral("stars/names.csv")));
...
void StarsPlugin::readSettings()
{
    ...
    Qt::CheckState const mercuryState = m_renderPlanet[QStringLiteral("mercury")] ? Qt::Checked : Qt::Unchecked;
    ui_configWidget->m_solarSystemListWidget->item(2)->setCheckState(mercuryState);

    Qt::CheckState const venusState = m_renderPlanet[QStringLiteral("venus")] ? Qt::Checked : Qt::Unchecked;
    ui_configWidget->m_solarSystemListWidget->item(3)->setCheckState(venusState);

    Qt::CheckState const marsState = m_renderPlanet[QStringLiteral("mars")] ? Qt::Checked : Qt::Unchecked;
    ui_configWidget->m_solarSystemListWidget->item(5)->setCheckState(marsState);

    Qt::CheckState const jupiterState = m_renderPlanet[QStringLiteral("jupiter")] ? Qt::Checked : Qt::Unchecked;
    ui_configWidget->m_solarSystemListWidget->item(6)->setCheckState(jupiterState);

    Qt::CheckState const saturnState = m_renderPlanet[QStringLiteral("saturn")] ? Qt::Checked : Qt::Unchecked;
    ui_configWidget->m_solarSystemListWidget->item(7)->setCheckState(saturnState);

    Qt::CheckState const uranusState = m_renderPlanet[QStringLiteral("uranus")] ? Qt::Checked : Qt::Unchecked;
    ui_configWidget->m_solarSystemListWidget->item(8)->setCheckState(uranusState);

    Qt::CheckState const neptuneState = m_renderPlanet[QStringLiteral("neptune")] ? Qt::Checked : Qt::Unchecked;
...
void StarsPlugin::writeSettings()
{
    ...
    m_renderPlanet[QStringLiteral("mercury")] = ui_configWidget->m_solarSystemListWidget->item(2)->checkState() == Qt::Checked;
    m_renderPlanet[QStringLiteral("venus")] = ui_configWidget->m_solarSystemListWidget->item(3)->checkState() == Qt::Checked;
    m_renderPlanet[QStringLiteral("mars")] = ui_configWidget->m_solarSystemListWidget->item(5)->checkState() == Qt::Checked;
    m_renderPlanet[QStringLiteral("jupiter")] = ui_configWidget->m_solarSystemListWidget->item(6)->checkState() == Qt::Checked;
    m_renderPlanet[QStringLiteral("saturn")] = ui_configWidget->m_solarSystemListWidget->item(7)->checkState() == Qt::Checked;
    m_renderPlanet[QStringLiteral("uranus")] = ui_configWidget->m_solarSystemListWidget->item(8)->checkState() == Qt::Checked;
    m_renderPlanet[QStringLiteral("neptune")] = ui_configWidget->m_solarSystemListWidget->item(9)->checkState() == Qt::Checked;
...
void StarsPlugin::loadStars()
{
    ...
    QFile starFile(MarbleDirs::dataFilePath(QStringLiteral("stars/stars.dat")));
    ...
    m_pixmapSun.load(MarbleDirs::dataFilePath(QStringLiteral("svg/sun.png")));
    m_pixmapMoon.load(MarbleDirs::dataFilePath(QStringLiteral("svg/moon.png")));
...
void StarsPlugin::createStarPixmaps()
{
    ...
    pixBigStars.clear();
    pixBigStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_0_blue.png"))));
    pixBigStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_0_bluewhite.png"))));
    pixBigStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_0_white.png"))));
    pixBigStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_0_yellow.png"))));
    pixBigStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_0_orange.png"))));
    pixBigStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_0_red.png"))));
    pixBigStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_0_garnetred.png"))));

    QList<QPixmap> pixSmallStars;
    pixSmallStars.clear();
    pixSmallStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_3_blue.png"))));
    pixSmallStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_3_bluewhite.png"))));
    pixSmallStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_3_white.png"))));
    pixSmallStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_3_yellow.png"))));
    pixSmallStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_3_orange.png"))));
    pixSmallStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_3_red.png"))));
    pixSmallStars.append(QPixmap(MarbleDirs::dataFilePath(QStringLiteral("bitmaps/stars/star_3_garnetred.png"))));
...
void StarsPlugin::loadConstellations()
{
    ...
    QFile constellationFile(MarbleDirs::dataFilePath(QStringLiteral("stars/constellations.dat")));
...
void StarsPlugin::loadDsos()
{
    ...
    QFile dsoFile(MarbleDirs::dataFilePath(QStringLiteral("stars/dso.dat")));
    ...
    m_dsoImage.load(MarbleDirs::dataFilePath(QStringLiteral("stars/deepsky.png")));
    m_dsosLoaded = true;
}

bool StarsPlugin::render(GeoPainter *painter, ViewportParams *viewport, const QString &renderPos, GeoSceneLayer *layer)
{
    Q_UNUSED(renderPos)
    Q_UNUSED(layer)
    ...
    const bool doRender = !viewport->mapCoversViewport()
        && ((viewport->projection() == Spherical || viewport->projection() == VerticalPerspective)
            && planetId == QLatin1StringView("earth")); // So far displaying stars is only supported on earth.

    if (doRender != m_doRender) {
        if (doRender) {
            connect(marbleModel()->clock(), SIGNAL(timeChanged()), this, SLOT(requestRepaint()));
        } else {
            disconnect(marbleModel()->clock(), SIGNAL(timeChanged()), this, SLOT(requestRepaint()));
        }
    ...
    sys.setCurrentMJD(dateTime.date().year(),
                      dateTime.date().month(),
                      dateTime.date().day(),
                      dateTime.time().hour(),
    ...
    if (doRender) {
        ...
        if (m_renderCelestialPole) {
            ...
            if (qpos1.v[Q_Z] < 0) {
                ...
                painter->drawText(x1 + 8, y1 + 12, QStringLiteral("NP"));
            }
            ...
            if (qpos2.v[Q_Z] < 0) {
                ...
                painter->drawLine(x1, y1, x1 + 10, y1);
                painter->drawLine(x1 + 5, y1 - 5, x1 + 5, y1 + 5);
                painter->drawText(x1 + 8, y1 + 12, QStringLiteral("SP"));
            }
        }
        ...
        if (m_renderDsos) {
            ...
            for (int d = 0; d < m_dsos.size(); ++d) {
                ...
                if (qpos.v[Q_Z] < 0 && ((earthCenteredX * earthCenteredX + earthCenteredY * earthCenteredY) < earthRadius * earthRadius)) {
                    continue;
                }
        ...
        if (m_renderConstellationLines || m_renderConstellationLabels) {
            ...
            for (int c = 0; c < m_constellations.size(); ++c) {
                ...
                if (endptCount < 1 || xMean < 0 || xMean >= viewport->width() || yMean < 0 || yMean >= viewport->height())
                    continue;
        ...
        if (m_renderSun) {
            ...
            if (qpos.v[Q_Z] <= 0) {
                QPixmap glow(MarbleDirs::dataFilePath(QStringLiteral("svg/glow.png")));
                ...
                if (!(x < -glow.width() || x >= viewport->width() || y < -glow.height() || y >= viewport->height())) {
                ...
                if (glowDrawn) {
                    ...
                    QPixmap sun = m_pixmapSun.transformed(QTransform().scale(factor, factor), Qt::SmoothTransformation);
                    deltaX = sun.width() / 2.;
                    deltaY = sun.height() / 2.;
        ...
        if (m_renderMoon && marbleModel()->planetId() == QLatin1StringView("earth")) {
            // moon
            double ra = 0.0;
            double decl = 0.0;
            sys.getMoon(ra, decl);
            ...
            if (qpos.v[Q_Z] <= 0) {
                ...
                if (!(x < -size || x >= viewport->width() || y < -size || y >= viewport->height())) {
                    // Moon phases
                    ...
                    if (viewport->polarity() < 0)
                        angle += 180;

                    QTransform form;
                    const qreal factor = size / moon.size().width();
                    moon = moon.transformed(form.rotate(angle).scale(factor, factor), Qt::SmoothTransformation);

                    painter->drawPixmap(x - deltaX, y - deltaY, moon);

                    // It's labels' time!
                    if (m_viewSolarSystemLabel)
                        painter->drawText(x + deltaX, y + deltaY, PlanetFactory::localizedName(QStringLiteral("moon")));
                }
            }
        }
...
void StarsPlugin::renderPlanet(const QString &planetId, GeoPainter *painter, SolarSystem &sys, ViewportParams *viewport, qreal skyRadius, matrix &skyAxisMatrix)
    const
{
...
{
    m_renderPlanet[QStringLiteral("venus")] = on;
    m_renderPlanet[QStringLiteral("mars")] = on;
    m_renderPlanet[QStringLiteral("jupiter")] = on;
    m_renderPlanet[QStringLiteral("mercury")] = on;
    m_renderPlanet[QStringLiteral("saturn")] = on;
    m_renderPlanet[QStringLiteral("uranus")] = on;
    m_renderPlanet[QStringLiteral("neptune")] = on;
    ...
    if (e->type() == QEvent::ContextMenu) {
        ...
        if (widget && menuEvent) {
            ...
            for (AbstractFloatItem *floatItem : widget->floatItems()) {
                if (floatItem->enabled() && floatItem->visible() && floatItem->contains(menuEvent->pos())) {
                    return false;
                }
            }
            ...
            if (!m_contextMenu) {
                ...
                m_constellationsAction = m_contextMenu->addAction(tr("Show &Constellations"), this, SLOT(toggleConstellations(bool)));
                m_constellationsAction->setCheckable(true);

                m_sunMoonAction = m_contextMenu->addAction(tr("Show &Sun and Moon"), this, SLOT(toggleSunMoon(bool)));
                m_sunMoonAction->setCheckable(true);

                m_planetsAction = m_contextMenu->addAction(tr("Show &Planets"), this, SLOT(togglePlanets(bool)));
                m_planetsAction->setCheckable(true);

                m_dsoAction = m_contextMenu->addAction(tr("Show &Deep Sky Objects"), this, SLOT(toggleDsos(bool)));
                m_dsoAction->setCheckable(true);

                m_contextMenu->addSeparator();
                m_contextMenu->addAction(tr("&Configure..."), this, SLOT(executeConfigDialog()));
            }
            ...
            const bool isAnyPlanetRendered = m_renderPlanet[QStringLiteral("venus")] || m_renderPlanet[QStringLiteral("mars")]
                || m_renderPlanet[QStringLiteral("jupiter")] || m_renderPlanet[QStringLiteral("mercury")] || m_renderPlanet[QStringLiteral("saturn")]
                || m_renderPlanet[QStringLiteral("uranus")] || m_renderPlanet[QStringLiteral("neptune")];
