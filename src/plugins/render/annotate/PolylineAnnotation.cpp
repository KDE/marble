...
void PolylineAnnotation::updateRegions(GeoPainter *painter)
{
    ...
    if (state() == SceneGraphicsItem::AddingNodes) {
        ...
        for (int i = 0; i < line.size() - 1; ++i) {
            const QRegion newRegion(painter->regionFromEllipse(line.at(i).interpolate(line.at(i + 1), 0.5), hoveredDim, hoveredDim));
    ...
    for (int i = 0; i < m_nodesList.size(); ++i) {
        const QRegion newRegion = m_nodesList.at(i).isSelected() ? painter->regionFromEllipse(line.at(i), selectedDim, selectedDim)
...
void PolylineAnnotation::drawNodes(GeoPainter *painter)
{
    ...
    for (int i = 0; i < line.size(); ++i) {
        ...
        if (m_nodesList.at(i).isBeingMerged()) {
            ...
            if (m_nodesList.at(i).isEditingHighlighted() || m_nodesList.at(i).isMergingHighlighted()) {
...
bool PolylineAnnotation::containsPoint(const QPoint &point) const
{
    if (state() == SceneGraphicsItem::Editing) {
        ...
        return virtualNodeContains(point) != -1 || nodeContains(point) != -1 || polylineContains(point);
    }
...
void PolylineAnnotation::dealWithItemChange(const SceneGraphicsItem *other)
{
    ...
    if (state() == SceneGraphicsItem::Editing) {
        if (m_hoveredNodeIndex != -1 && m_hoveredNodeIndex < static_cast<GeoDataLineString *>(placemark()->geometry())->size()) {
...
void PolylineAnnotation::changeClickedNodeSelection()
{
    ...
    m_nodesList[m_clickedNodeIndex].setFlag(PolylineNode::NodeIsSelected, !m_nodesList[m_clickedNodeIndex].isSelected());
}
...
bool PolylineAnnotation::processEditingOnRelease(QMouseEvent *mouseEvent)
{
    ...
    if (m_interactingObj == InteractingNode) {
        ...
        m_nodesList[m_clickedNodeIndex].setFlag(PolylineNode::NodeIsSelected, !m_nodesList.at(m_clickedNodeIndex).isSelected());
        m_interactingObj = InteractingNothing;
        return true;
    } else if (m_interactingObj == InteractingPolyline) {
...
bool PolylineAnnotation::dealWithHovering(QMouseEvent *mouseEvent)
{
    ...
    const int index = nodeContains(mouseEvent->pos());
    if (index != -1) {
        if (!m_nodesList.at(index).isEditingHighlighted() && !m_nodesList.at(index).isMergingHighlighted()) {
