        ...
        for (int i = NorthWest; i != Polygon; ++i) {
            ...
            if (m_editStatus == Resize) {
                if (m_hoveredHandle != i) {
                    painter->drawImage(coordinateList.at(i), m_resizeIcons.at(2 * i).transformed(trans, Qt::SmoothTransformation));
                } else {
                    painter->drawImage(coordinateList.at(i), m_resizeIcons.at(2 * i + 1).transformed(trans, Qt::SmoothTransformation));
                }
            } else if (m_editStatus == Rotate) {
                if (m_hoveredHandle != i) {
                    painter->drawImage(coordinateList.at(i), m_rotateIcons.at(2 * i).transformed(trans, Qt::SmoothTransformation));
                } else {
                    painter->drawImage(coordinateList.at(i), m_rotateIcons.at(2 * i + 1).transformed(trans, Qt::SmoothTransformation));
                }
            }
        }
    }
...
bool GroundOverlayFrame::containsPoint(const QPoint &eventPos) const
{
    ...
    return m_movedHandle != NoRegion || m_hoveredHandle != NoRegion;
}
...
bool GroundOverlayFrame::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (geodata_cast<GeoDataPolygon>(placemark()->geometry())) {
        ...
        if (m_editStatus == Resize) {
            ...
            if (m_overlay->latLonBox().rotation() != 0) {
                rotatedCoord = coord.rotateAround(m_overlay->latLonBox().center(), -m_overlay->latLonBox().rotation());
            }
            ...
            if (m_movedHandle != Polygon) {
                ...
                qreal angle1 = qAtan2(event->pos().y() - center.y(), event->pos().x() - center.x());
                qreal angle2 = qAtan2(m_movedHandleScreenCoordinates.y() - center.y(), m_movedHandleScreenCoordinates.x() - center.x());
