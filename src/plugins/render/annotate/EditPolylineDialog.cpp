...
EditPolylineDialog::EditPolylineDialog(GeoDataPlacemark *placemark, const QHash<qint64, OsmPlacemarkData> *relations, QWidget *parent)
    : QDialog(parent)
    , d(new Private(placemark))
{
    ...
    if (relations) {
        ...
        QObject::connect(d->m_osmTagEditorWidget, SIGNAL(placemarkChanged(GeoDataFeature *)), this, SLOT(updatePolyline()));
    ...
    connect(d->m_delegate, SIGNAL(modelChanged(GeoDataPlacemark *)), this, SLOT(handleItemMoving(GeoDataPlacemark *)));
    connect(d->m_delegate, SIGNAL(geometryChanged()), this, SLOT(updatePolyline()));
...
void EditPolylineDialog::checkFields()
{
    ...
    if (d->m_name->text().isEmpty()) {
        QMessageBox::warning(this, tr("No name specified"), tr("Please specify a name for this polyline."));
        ok = false;
    } else {
        if (const auto lineString = geodata_cast<GeoDataLineString>(d->m_placemark->geometry())) {
            if (lineString->size() < 2) {
                QMessageBox::warning(this, tr("Not enough nodes specified."), tr("Please specify at least 2 nodes for the path by clicking on the map."));
                ok = false;
            }
        }
    }
