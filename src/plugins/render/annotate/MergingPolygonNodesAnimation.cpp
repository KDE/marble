...
{
    return m_boundary == OuterBoundary ? outerRing.at(first_i).interpolate(outerRing.at(second_i), 0.5)
...
{
    return m_boundary == OuterBoundary ? outerRing.at(first_i).sphericalDistanceTo(outerRing.at(second_i))
