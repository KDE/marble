...
EditPolygonDialog::EditPolygonDialog(GeoDataPlacemark *placemark, const QHash<qint64, OsmPlacemarkData> *relations, QWidget *parent)
    : QDialog(parent)
    , d(new Private(placemark))
{
    ...
    connect(d->m_delegate, SIGNAL(geometryChanged()), this, SLOT(updatePolygon()));
...
void EditPolygonDialog::checkFields()
{
    ...
    if (d->m_name->text().isEmpty()) {
        QMessageBox::warning(this, tr("No name specified"), tr("Please specify a name for this polygon."));
        ok = false;
    } else {
        if (const auto polygon = geodata_cast<GeoDataPolygon>(d->m_placemark->geometry())) {
            if (polygon->outerBoundary().size() < 3) {
                QMessageBox::warning(this, tr("Not enough nodes specified."), tr("Please specify at least 3 nodes for the polygon by clicking on the map."));
                ok = false;
            }
        }
    }
