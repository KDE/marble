...
void RoutingPluginPrivate::updateZoomButtons(int zoomValue)
{
    ...
    if ((zoomInEnabled != m_widget.zoomInButton->isEnabled()) || (zoomOutEnabled != m_widget.zoomOutButton->isEnabled())) {
...
void RoutingPluginPrivate::toggleGuidanceMode(bool enabled)
{
    ...
    if (enabled) {
        QObject::connect(m_routingModel, SIGNAL(positionChanged()), m_parent, SLOT(updateDestinationInformation()));
    } else {
        QObject::disconnect(m_routingModel, SIGNAL(positionChanged()), m_parent, SLOT(updateDestinationInformation()));
    }
...
void RoutingPlugin::initialize()
{
    ...
    connect(marbleModel()->positionTracking(),
            ...
            this,
            SLOT(updateGpsButton(PositionProviderPlugin *)));
...
bool RoutingPlugin::eventFilter(QObject *object, QEvent *e)
{
    ...
    if (widget && !d->m_marbleWidget) {
        ...
        connect(d->m_widget.routingButton, SIGNAL(clicked(bool)), this, SLOT(toggleGuidanceMode(bool)));
        connect(d->m_widget.gpsButton, SIGNAL(clicked(bool)), this, SLOT(togglePositionTracking(bool)));
        connect(d->m_widget.zoomInButton, SIGNAL(clicked()), d->m_marbleWidget, SLOT(zoomIn()));
        connect(d->m_widget.zoomOutButton, SIGNAL(clicked()), d->m_marbleWidget, SLOT(zoomOut()));
        connect(d->m_marbleWidget, SIGNAL(themeChanged(QString)), this, SLOT(updateZoomButtons()));
        connect(d->m_marbleWidget, SIGNAL(zoomChanged(int)), this, SLOT(updateZoomButtons(int)));
        connect(d->m_routingModel, SIGNAL(currentRouteChanged()), this, SLOT(updateGuidanceModeButton()));
