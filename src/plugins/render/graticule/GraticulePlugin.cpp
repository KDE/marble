                                 ...
                                 const QPen &gridCirclePen)
{
    ...
    if (marbleModel()->planet()->id() != QLatin1StringView("sky") && notation != GeoDataCoordinates::Astro) {
    ...
    if (m_currentNotation == GeoDataCoordinates::UTM) {
        ...
        renderLongitudeLines(painter, viewLatLonAltBox, 6.0, 0.0, 18.0, 154.0, LineEnd | IgnoreXMargin);
        renderLongitudeLines(painter, viewLatLonAltBox, 6.0, 0.0, 34.0, 10.0, LineStart | IgnoreXMargin);

        // Paint longitudes with exceptions
        renderLongitudeLines(painter, viewLatLonAltBox, 6.0, 0.0, 6.0, 162.0, LineEnd | IgnoreXMargin);
        renderLongitudeLines(painter, viewLatLonAltBox, 6.0, 0.0, 26.0, 146.0, LineEnd | IgnoreXMargin);
    ...
    renderLongitudeLines(painter, viewLatLonAltBox, normalDegreeStep, boldDegreeStep, normalDegreeStep, normalDegreeStep, labelXPosition);
    renderLatitudeLines(painter, viewLatLonAltBox, normalDegreeStep, boldDegreeStep, labelYPosition);
    ...
    renderLongitudeLines(painter, viewLatLonAltBox, boldDegreeStep, 0.0, normalDegreeStep, normalDegreeStep, NoLabel);

    renderLatitudeLines(painter, viewLatLonAltBox, boldDegreeStep, 0.0, NoLabel);

    QPen tropicsPen = tropicsCirclePen;
    if (painter->mapQuality() != OutlineQuality && painter->mapQuality() != LowQuality) {
...
void GraticulePlugin::renderLatitudeLine(GeoPainter *painter,
                                         qreal latitude,
                                          ...
                                          LabelPositionFlags labelPositionFlags)
{
    ...
    // Coordinate line is not displayed inside the viewport
    if ((!viewLatLonAltBox.crossesDateLine() && (longitude * DEG2RAD < fromWestLon || toEastLon < longitude * DEG2RAD))
        || (viewLatLonAltBox.crossesDateLine() && longitude * DEG2RAD < toEastLon && fromWestLon < longitude * DEG2RAD && fromWestLon != -M_PI
            && toEastLon != +M_PI)) {
                                          ...
                                          LabelPositionFlags labelPositionFlags)
{
    ...
    while (itStep < northLineLat) {
        ...
        QString label = GeoDataCoordinates::latToString(itStep, notation, GeoDataCoordinates::Degree, -1, 'g');
                                          ...
                                          LabelPositionFlags labelPositionFlags)
{
    ...
    if (northPolarGap == 6.0 && southPolarGap == 162.0) {
        if (label == QLatin1StringView("33")) {
            renderLongitudeLine(painter, itStep - 3.0, viewLatLonAltBox, northPolarGap, southPolarGap, label, labelPositionFlags);
        } else if (label == QLatin1StringView("35")) {
            renderLongitudeLine(painter, itStep - 3.0, viewLatLonAltBox, northPolarGap, southPolarGap, label, labelPositionFlags);
        } else if (label == QLatin1StringView("37")) {
            renderLongitudeLine(painter, itStep - 3.0, viewLatLonAltBox, northPolarGap, southPolarGap, label, labelPositionFlags);
        } else if (label == QLatin1StringView("32") || label == QLatin1StringView("34") || label == QLatin1StringView("36")) {
            // paint nothing
        } else {
            renderLongitudeLine(painter, itStep, viewLatLonAltBox, northPolarGap, southPolarGap, label, labelPositionFlags);
        }
    } else if (northPolarGap == 26.0 && southPolarGap == 146.0) {
        if (label == QLatin1StringView("32")) {
            renderLongitudeLine(painter, itStep - 3.0, viewLatLonAltBox, northPolarGap, southPolarGap, label, labelPositionFlags);
        } else {
            renderLongitudeLine(painter, itStep, viewLatLonAltBox, northPolarGap, southPolarGap, label, labelPositionFlags);
        }
                                           ...
                                           LabelPositionFlags labelPositionFlags) const
{
    ...
    const bool isSky = (marbleModel()->planet()->id() == QLatin1StringView("sky"));
    ...
    if (!viewLatLonAltBox.crossesDateLine() || (westLon == -180.0 && eastLon == +180.0)) {
        qreal itStep = westLineLon;

        while (itStep < eastLineLon) {
            // Create a matching label
            QString label = GeoDataCoordinates::lonToString(itStep, notation, GeoDataCoordinates::Degree, precision, 'g');
            ...
            if (notation == GeoDataCoordinates::UTM) {
                renderUtmExceptions(painter, viewLatLonAltBox, itStep, northPolarGap, southPolarGap, label, labelPositionFlags);
