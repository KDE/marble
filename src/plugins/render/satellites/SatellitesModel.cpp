...
SatellitesModel::SatellitesModel(GeoDataTreeModel *treeModel, const MarbleClock *clock)
...
void SatellitesModel::parseFile(const QString &id, const QByteArray &data)
{
...
void SatellitesModel::parseTLE(const QString &id, const QByteArray &data)
{
    ...
    while (i < tleLines.size() - 1) {
        ...
        twoline2rv(line1, line2, 'c', 'd', 'i', wgs84, startmfe, stopmfe, deltamin, satrec);
        if (satrec.error != 0) {
