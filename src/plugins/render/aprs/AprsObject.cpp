...
//
// SPDX-FileCopyrightText: 2010 Wes Hardaker <marble@ws6z.com>
//
...
QColor AprsObject::calculatePaintColor(int from, const QElapsedTimer &time, int fadeTime)
{
    QColor color;
...
void AprsObject::render(GeoPainter *painter, ViewportParams *viewport, int fadeTime, int hideTime)
{
    Q_UNUSED(viewport);
    ...
    QColor baseColor = calculatePaintColor(m_seenFrom, m_history.last().timestamp(), fadeTime);
