...
AprsGatherer::AprsGatherer(AprsSource *from, QMap<QString, AprsObject *> *objects, QMutex *mutex, QString *filter)
    : m_source(from)
    , m_socket(nullptr)
    , m_filter(filter)
    , m_running(true)
...
AprsGatherer::AprsGatherer(QIODevice *from, QMap<QString, AprsObject *> *objects, QMutex *mutex, QString *filter)
    : m_source(nullptr)
    , m_socket(from)
    , m_filter(filter)
    , m_running(true)
...
void AprsGatherer::run()
{
    ...
    while (m_running) {
        ...
        if (!m_socket) {
            mDebug() << "aprs: failed to open socket from " << m_sourceName.toLocal8Bit().data();
            sleep(5);
            continue;
        }
        ...
        if (m_socket && m_filter != nullptr) {
            ...
            if (m_filter->length() > 0) {
                m_socket->write(m_filter->toLocal8Bit().data(), m_filter->length());
            }
        }
        ...
        if (m_dumpOutput)
            mDebug() << "aprs: " << m_sourceName.toLocal8Bit().data() << ": " << line;
        ...
        if (matcher.indexIn(line) != -1) {
            ...
            qreal latitude =
                // hours
                m_dstCallDigits[dstCall[0]] * 10 + m_dstCallDigits[dstCall[1]] +

                // minutes
                (qreal(m_dstCallDigits[dstCall[2]] * 10 + m_dstCallDigits[dstCall[3]]) + qreal(m_dstCallDigits[dstCall[4]]) / 10.0
                 + qreal(m_dstCallDigits[dstCall[5]]) / 100)
                    / 60.0;

            if (m_dstCallSouthEast[dstCall[4]])
                latitude = -latitude;

            qreal longitude = calculateLongitude(QString(mic_e_matcher.cap(4)), m_dstCallLongitudeOffset[dstCall[4]], m_dstCallSouthEast[dstCall[5]]);

            //           mDebug() << "  MIC-E: " << line.toLocal8Bit().data();
            //           mDebug() << "    lat: " << latitude;
            //           mDebug() << "    lon: " << longitude;

            addObject(myCall, latitude, longitude, canDoDirect, QString(mic_e_matcher.cap(3)), QChar(mic_e_matcher.cap(6)[1]), QChar(mic_e_matcher.cap(6)[0]));
        } else {
...
void AprsGatherer::shutDown()
{
    m_running = false;
}

void AprsGatherer::addObject(const QString &callSign,
                             qreal latitude,
                             qreal longitude,
                             bool canDoDirect,
...
qreal AprsGatherer::calculateLongitude(const QString &threeBytes, int offset, bool isEast)
{
    ...
    hours += (qreal((threeBytes[1].toLatin1() - 28) % 60) + (qreal(threeBytes[2].toLatin1() - 28)) / 100) / 60.0;
...
void AprsGatherer::setDumpOutput(bool to)
{
    m_dumpOutput = to;
}

bool AprsGatherer::dumpOutput() const
{
    return m_dumpOutput;
}

void AprsGatherer::setSeenFrom(GeoAprsCoordinates::SeenFrom to)
{
    m_seenFrom = to;
}

GeoAprsCoordinates::SeenFrom AprsGatherer::seenFrom()
{
    return m_seenFrom;
}

// gets around parent QThread protected sleep as our objects need a sleep
void AprsGatherer::sleepFor(int seconds)
{
    sleep(seconds);
}
