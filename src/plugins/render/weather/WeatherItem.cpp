...
public:
    ...
    void updateToolTip()
    {
        ...
        if (m_currentWeather.hasValidCondition() && m_currentWeather.hasValidTemperature())
            toolTip += QStringLiteral("%2, %3\n").arg(m_currentWeather.conditionString()).arg(temperatureString());
        ...
        if (m_currentWeather.hasValidWindSpeed() && m_currentWeather.hasValidWindDirection())
            toolTip += tr("Wind: %4, %5\n", "Wind: WindSpeed, WindDirection").arg(windSpeedString()).arg(m_currentWeather.windDirectionString());
        else if (m_currentWeather.hasValidWindSpeed())
            toolTip += tr("Wind: %4\n", "Wind: WindSpeed").arg(m_currentWeather.windSpeedString());
        ...
        if (m_currentWeather.hasValidPressure() && m_currentWeather.hasValidPressureDevelopment())
            toolTip += tr("Pressure: %6, %7", "Pressure: Pressure, Development").arg(pressureString()).arg(m_currentWeather.pressureDevelopmentString());
        else if (m_currentWeather.hasValidPressure())
            toolTip += tr("Pressure: %6", "Pressure: Pressure").arg(pressureString());
        else if (m_currentWeather.hasValidPressureDevelopment())
            toolTip += tr("Pressure %7", "Pressure Development").arg(m_currentWeather.pressureDevelopmentString());
    ...
    void updateFavorite()
    {
        QStringList items = m_settings.value(QStringLiteral("favoriteItems")).toString().split(QLatin1Char(','), Qt::SkipEmptyParts);
        bool favorite = items.contains(m_parent->id());

        m_favoriteButton.setVisible(favorite);
        m_favoriteAction.setText(favorite ? tr("Remove from Favorites") : tr("Add to Favorites"));
    ...
    {
        return m_currentWeather.hasValidCondition() && m_settings.value(QStringLiteral("showCondition"), showConditionDefault).toBool();
    }

    bool isTemperatureShown() const
    {
        return m_currentWeather.hasValidTemperature() && m_settings.value(QStringLiteral("showTemperature"), showTemperatureDefault).toBool();
    }

    bool isWindDirectionShown() const
    {
        return m_currentWeather.hasValidWindDirection() && m_settings.value(QStringLiteral("showWindDirection"), showWindDirectionDefault).toBool();
    }

    bool isWindSpeedShown() const
    {
        return m_currentWeather.hasValidWindSpeed() && m_settings.value(QStringLiteral("showWindSpeed"), showWindSpeedDefault).toBool();
    }
    ...
    {
        WeatherData::TemperatureUnit tUnit = (WeatherData::TemperatureUnit)m_settings.value(QStringLiteral("temperatureUnit"), WeatherData::Celsius).toInt();
        return tUnit;
    }
    ...
    {
        return (WeatherData::SpeedUnit)m_settings.value(QStringLiteral("windSpeedUnit"), WeatherData::kph).toInt();
    }
    ...
    {
        return (WeatherData::PressureUnit)m_settings.value(QStringLiteral("pressureUnit"), WeatherData::HectoPascal).toInt();
    }
...
{
    disconnect(&d->m_browserAction, SIGNAL(triggered()), this, SLOT(openBrowser()));
    connect(&d->m_browserAction, SIGNAL(triggered()), this, SLOT(openBrowser()));
...
{
    return d->isConditionShown() || d->isTemperatureShown() || d->isWindDirectionShown() || d->isWindSpeedShown();
}
...
QString WeatherItem::createFromTemplate(const QString &templateHtml)
{
    ...
    for (const WeatherData &forecast : std::as_const(d->m_forecastWeather)) {
        ...
        html.replace(QLatin1StringView("%day_f") + suffix + QLatin1Char('%'), locale.standaloneDayName(date.dayOfWeek()));
        html.replace(QLatin1StringView("%weather_situation_f") + suffix + QLatin1Char('%'), QLatin1StringView("file://") + forecast.iconSource());
        html.replace(QLatin1StringView("%max_temp_f") + suffix + QLatin1Char('%'), forecast.maxTemperatureString(WeatherData::Celsius));
        html.replace(QLatin1StringView("%min_temp_f") + suffix + QLatin1Char('%'), forecast.minTemperatureString(WeatherData::Celsius));
        html.replace(QLatin1StringView("%condition_f") + suffix + QLatin1Char('%'), forecast.conditionString());
        html.replace(QLatin1StringView("%wind_direction_f") + suffix + QLatin1Char('%'), forecast.windDirectionString());
        html.replace(QLatin1StringView("%wind_speed_f") + suffix + QLatin1Char('%'), forecast.windSpeedString());
        html.replace(QLatin1StringView("%publish_time_f") + suffix + QLatin1Char('%'), forecast.publishingTime().toString());
    }
...
QList<QAction *> WeatherItem::actions()
{
    ...
    disconnect(&d->m_favoriteAction, SIGNAL(triggered()), this, SLOT(toggleFavorite()));
...
{
    return d->m_currentWeather.hasValidTemperature() ? d->m_currentWeather.temperature(WeatherData::Celsius) : 0.0;
}
