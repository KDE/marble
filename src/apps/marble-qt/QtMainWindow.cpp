    ...
    , m_manageBookmarksAction(nullptr)
{
    ...
    m_configDialog = new QtMarbleConfigDialog(m_controlView->marbleWidget(), m_controlView->cloudSyncManager(), this);
    connect(m_configDialog, SIGNAL(settingsChanged()), this, SLOT(updateSettings()));
    connect(m_configDialog, SIGNAL(clearVolatileCacheClicked()), m_controlView->marbleWidget(), SLOT(clearVolatileTileCache()));
    connect(m_configDialog, SIGNAL(clearPersistentCacheClicked()), m_controlView->marbleModel(), SLOT(clearPersistentTileCache()));
    connect(m_configDialog, SIGNAL(syncNowClicked()), m_controlView->cloudSyncManager()->bookmarkSyncManager(), SLOT(startBookmarkSync()));
    connect(m_configDialog, SIGNAL(syncNowClicked()), m_configDialog, SLOT(disableSyncNow()));

    connect(m_controlView->marbleModel()->fileManager(), &FileManager::fileError, this, [this](const QString &path, const QString &error) {
        QMessageBox::warning(this,
                             tr("Marble"), // krazy:exclude=qclasses
    ...
    connect(m_controlView->marbleModel(), SIGNAL(themeChanged(QString)), this, SLOT(updateWindowTitle()));
    connect(m_controlView->marbleModel(), SIGNAL(themeChanged(QString)), this, SLOT(updateCenterFromTheme()));
    ...
    QMetaObject::invokeMethod(this, "initObject", Qt::QueuedConnection, Q_ARG(QVariantMap, cmdLineSettings));
}
...
void MainWindow::createActions()
{
    ...
    connect(m_toggleTileLevelAction, SIGNAL(triggered(bool)), this, SLOT(showZoomLevel(bool)));
    ...
    connect(m_angleDisplayUnitActionGroup, SIGNAL(triggered(QAction *)), this, SLOT(changeAngleDisplayUnit(QAction *)));
...
void MainWindow::createBookmarkMenu()
{
    ...
    m_bookmarkMenu->addAction(QIcon(QStringLiteral(":/icons/go-home.png")), tr("&Home"), m_controlView->marbleWidget(), SLOT(goHome()));
...
void MainWindow::createFolderList(QMenu *bookmarksListMenu, const GeoDataContainer *container)
{
    ...
    connect(bookmarksListMenu, SIGNAL(triggered(QAction *)), this, SLOT(lookAtBookmark(QAction *)));
}
...
void MainWindow::exportMapScreenShot()
{
    QString fileName = QFileDialog::getSaveFileName(this,
                                                    tr("Export Map"), // krazy:exclude=qclasses
                                                    QDir::homePath(),
                                                    tr("Images (*.jpg *.png)"));
...
void MainWindow::setupStatusBar()
{
    ...
    int maxDateTimeWidth = fontMetrics().boundingRect(templateDateTimeString).width() + 2 * m_clockLabel->margin() + 2 * m_clockLabel->indent();
    m_clockLabel->setFixedWidth(maxDateTimeWidth);
    statusBar()->addPermanentWidget(m_clockLabel);

    connect(marbleWidget(), SIGNAL(mouseMoveGeoPosition(QString)), this, SLOT(showPosition(QString)));
    connect(marbleWidget(), SIGNAL(distanceChanged(QString)), this, SLOT(showDistance(QString)));
    connect(marbleWidget(), SIGNAL(tileLevelChanged(int)), this, SLOT(showZoom(int)));
...
void MainWindow::readSettings(const QVariantMap &overrideSettings)
{
    ...
    m_controlView->marbleWidget()->setProjection((Projection)(settings.value("projection", Spherical).toInt()));

    // Set home position
    m_controlView->marbleModel()->setHome(settings.value("homeLongitude", 9.4).toDouble(),
    ...
    if (lonLatIt != overrideSettings.constEnd()) {
        ...
        case Marble::LastLocationVisited:
            m_controlView->marbleWidget()->centerOn(settings.value("quitLongitude", 0.0).toDouble(), settings.value("quitLatitude", 0.0).toDouble());
...
void MainWindow::updateSettings()
{
    ...
    */
    ...
    CloudSyncManager *cloudSyncManager = m_controlView->cloudSyncManager();
    cloudSyncManager->setOwncloudCredentials(m_configDialog->owncloudServer(), m_configDialog->owncloudUsername(), m_configDialog->owncloudPassword());
...
void MainWindow::showDownloadRegionDialog()
{
    ...
    ViewportParams const *const viewport = m_controlView->marbleWidget()->viewport();
...
void MainWindow::updateMapEditButtonVisibility(const QString &mapTheme)
{
    ...
    QStringList osmThemes = QStringList() << "earth/openstreetmap/openstreetmap.dgml"
