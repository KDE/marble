...
public:
    ...
    QImage tile(int n, int m, int maxTileLevel) override
    {
        ...
        if (hgtFileName(std::floor(startLng), std::floor(startLat)).isNull() && hgtFileName(std::floor(startLng) + 1, std::floor(startLat)).isNull()
        ...
        image = image.scaled(QSize(imageSizeResized, imageSizeResized), Qt::IgnoreAspectRatio);
...
private:
    QString hgtFileName(int lng, int lat) const
    {
        ...
        for (const QString &dir : dirs) {
            ...
            if (lat < 0)
                lat *= -1;
            fileName += QString("%1%2%3%4.hgt")
                            .arg(NS)
                            .arg(lat < 0 ? lat * -1 : lat, 2, 10, QLatin1Char('0'))
                            .arg(EW)
