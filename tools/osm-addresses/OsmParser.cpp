...
void Way::setRegion(const QHash<int, Node> &database, const OsmRegionTree &tree, QList<OsmOsmRegion> &osmOsmRegions, OsmPlacemark &placemark) const
{
    if (!city.isEmpty()) {
        ...
        for (const Node &node : database) {
            if (node.category >= OsmPlacemark::PlacesRegion && node.category <= OsmPlacemark::PlacesIsland && node.name == city) {
...
void OsmParser::read(const QFileInfo &content, const QString &areaName)
{
    ...
    GeoDataLatLonBox center(minLat.second, minLat.first, minLon.second, minLon.first);
...
bool OsmParser::shouldSave(ElementType /*type*/, const QString &key, const QString &value)
{
    ...
    return interestingTags.contains(key) && interestingTags[key].contains(value);
}
...
GeoDataLinearRing *OsmParser::convexHull() const
{
    ...
    for (int i = 0; i < points.size(); ++i) {
        points[i].direction = atan2(start.lon - points[i].coordinate.lon, start.lat - points[i].coordinate.lat);
    }
...
void OsmParser::writeKml(const QString &area,
...
Coordinate::Coordinate(float lon_, float lat_)
    : lon(lon_)
    , lat(lat_)
{
    // nothing to do
