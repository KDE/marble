...
void SpellChecker::correctPlaceLabels(const QVector<GeoDataPlacemark *> &placemarks)
{
    ...
    for (auto place : places) {
        ...
        if (!places.isEmpty()) {
            ...
            qDebug() << "No match for " << placeName << " at " << place->coordinate().toString(GeoDataCoordinates::Decimal)
                     << " and no candidates for replacement";
        }
        hits += hasMatch ? 1 : 0;
...
QVector<GeoDataPlacemark *> SpellChecker::candidatesFor(GeoDataPlacemark *placemark) const
{
    ...
    std::sort(places.begin(), places.end(), [placeName](GeoDataPlacemark *a, GeoDataPlacemark *b) {
